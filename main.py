...
class EditQuestionScreen(Screen):
    def set_context(self, ticket_name, question_index):
        ...
        if question_index < len(questions):
            ...
            self.ids.question_input.text = f"Запитання {question_index + 1}" # Нове питання за замовчуванням
    ...
    def save_question(self):
        ...
        save_data(data)
        print(f"Запитання {self.question_index + 1} білета '{self.ticket_name}' збережено!")
...
class MainScreen(Screen):
    ...
    def add_ticket_widget(self, name):
        ...
        btn = Button(text=name, size_hint_x=0.7)
        btn.bind(on_press=lambda x, n=name: self.view_ticket_questions(n))

        edit_btn = Button(text="Редагувати", size_hint_x=0.3)
        edit_btn.bind(on_press=lambda x, n=name: self.edit_ticket_prompt(n))
...
class AddTicketScreen(Screen):
    ...
    questions_inputs = ObjectProperty(None) # Додаємо ObjectProperty для BoxLayout з питаннями
    ...
    def save_ticket(self):
        ...
        self.manager.get_screen("main_screen").load_tickets() # Оновлюємо список білетів
...
class TicketQuestionsScreen(Screen):
    def set_ticket(self, ticket_name):
        self.ticket_name = ticket_name
        self.ids.ticket_title.text = f"[b]{ticket_name}[/b]" # Оновлюємо заголовок екрану
        ...
        while len(questions) < 6:
            questions.append({"text": f"Запитання {len(questions) + 1}", "answer_text": "", "answer_image": ""})
        ...
        for i, q_data in enumerate(questions):
            ...
            filled = bool(question_text.strip() or answer_text.strip() or answer_image.strip())

            q_display_text = question_text if question_text.strip() else f"Питання {i+1} (не заповнено)"
